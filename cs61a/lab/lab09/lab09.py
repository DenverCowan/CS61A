...
# if you use x.insert() you get a can't concatenate Nonetype error.
...
def non_decrease_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:       # list is empty
            return [[]]
        elif s[0] < prev:   # function is decreasing, don't include current element
            return subseq_helper(s[1:], prev)
        else:   # list is increasing, recursively call to check the rest of list using current element, and not using it
            a = subseq_helper(s[1:], s[0])  # include current element
            b = subseq_helper(s[1:], prev)  # exclude exclude current element
            return insert_into_all(s[0], a) + b     # combine results of recursive calls
    return subseq_helper(s, 0)      # initialize the process using a non positive number
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    answer = 0
    for i in range(1, n):
        answer += num_trees(i) * num_trees(n-i)
    return answer
