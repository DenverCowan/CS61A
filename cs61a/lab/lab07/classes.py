...
class Card:
    ...
    # int attack
    # int defense
    # must use self.name, self,attack, self.defense a.k.a. instance attributes
    # must implement power method which takes takes in another card as a parameter
...
class Player:
    ...
    def __init__(self, deck, name):
        ...
        "*** YOUR CODE HERE ***"
        self.hand = []
        # list += [value], list.append(value). no need for brackets around value with .append method.
        for _ in range(5):
    ...
    def draw(self):
        ...
        "*** YOUR CODE HERE ***"
        # append is much faster and less memory wasting than self.hand += self.deck.draw
        # .append and .extend will mutate the list in place, where += will make an entirely
        # new copy of the list.
        self.hand.append(self.deck.draw())

    def play(self, index):
        # index of card to remove from list
        ...
        """
        "*** YOUR CODE HERE ***"
        # .remove will remove the first occurrence of the item, whereas
        # .pop will remove the item at the given index.
...
class AICard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        # self here references the AI card
        # we want to add the top two cards to players hand.
        implemented = False
        for _ in range(2):
            player.draw()
        implemented = True
...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        # will add a copy of first card in hand, to current hand
        # will automatically lose round
        # if hand empty nothing is added but round is still lost
        added = False
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        """
        "*** YOUR CODE HERE ***"
        # if hand empty do nothing
        # discard card with highest power from hand and add its attack and defense to the played
        # card
        best_card = None
        if len(player.hand) > 0:
            # will return best card based on which one has the highest power vs opponent_card
            # you cannot compare a card to a card by themselves as they are just objects.
            # but you can compare the integer value you get from the power function of each card
            # which is where the key/lambda c function below comes in.
...
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        """
        "*** YOUR CODE HERE ***"
        # if attack or defense not negative at end of round card continues to the next round
        # cards attack and defense is permanently reduced by 1000 each, each round icluding the first
        re_add = False
        self.attack -= 1000
        self.defense -= 1000
        # below comparison has nothing to do with power function, only the above reduction of its
        # attributes.
