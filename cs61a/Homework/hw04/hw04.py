...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    True
    """
    # base case is simple enough, but the recursive call and return statement take a large
    # Recursive leap of faith.
    if is_leaf(t) and label(t) == "loki":
        return tree(lokis_replacement)
    else:
        bs = [replace_loki_at_leaf(b, lokis_replacement) for b in branches(t)]
        return tree(label(t), bs) # keeps OG label but now with corrected branches.
...
def has_path(t, word):
    ...
    if label(t) != word[0]: # if first letters don't match there is no path
        return False
    elif len(word) == 1: # if first letters match and there is only one letter in the word, then our work is done.
        return True
    else:
        # iterate through all branches, if branch has target letter, then
        for b in branches(t):
